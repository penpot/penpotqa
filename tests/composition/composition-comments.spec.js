...
mainTest.describe(() => {
  ...
  mainTest(qase(2086, 'Only your mentions filter'), async ({ page }, testInfo) => {
    await testInfo.setTimeout(testInfo.timeout + 60000);
    const firstEditor = random().concat('autotest');
    const firstEmail = `${process.env.GMAIL_NAME}+${firstEditor}@gmail.com`;
    const comment = 'Test Comment (main user)';
    ...
    await expect(page).toHaveScreenshot('only-your-mentions.png', {
      mask: [
        commentsPanelPage.commentsAuthorSection,
        commentsPanelPage.commentMentionText,
        mainPage.usersSection,
        mainPage.toolBarWindow,
      ],
    });
