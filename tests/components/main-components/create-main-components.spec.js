...
mainTest("Drag a component from assets tab and drop into workspace", async ({ page, browserName }) => {
...
mainTest("Create multiple components from rectangle and ellipse", async ({ page }) => {
  const mainPage = new MainPage(page);
  const layersPanelPage = new LayersPanelPage(page);
  await mainPage.createDefaultRectangleByCoordinates(200, 300);
  ...
  await mainPage.waitForChangeIsSaved();
  await expect(mainPage.viewport).toHaveScreenshot("multiple-components-canvas.png");
  await expect(layersPanelPage.layersSidebar).toHaveScreenshot("multiple-components-layer.png");
  await mainPage.openAssetsTab();
  await mainPage.expandComponentsBlockOnAssetsTab();
  await expect(mainPage.assetsPanel).toHaveScreenshot("multiple-components-asset.png");
});

mainTest("Create multiple components from text, curve and image", async ({ page, browserName }) => {
  const mainPage = new MainPage(page);
  const layersPanelPage = new LayersPanelPage(page);
  await mainPage.createDefaultTextLayer(browserName);
  await mainPage.createDefaultCurveLayer();
  ...
  await mainPage.createComponentsMultipleShapesRightClick(false);
  await mainPage.waitForChangeIsSaved();
  await expect(mainPage.viewport).toHaveScreenshot("multiple-components-canvas-3-layers.png");
  await expect(layersPanelPage.layersSidebar).toHaveScreenshot("multiple-components-layer-3-layers.png");
  await mainPage.openAssetsTab();
  await mainPage.expandComponentsBlockOnAssetsTab();
  await expect(mainPage.assetsPanel).toHaveScreenshot("multiple-components-asset-3-layers.png");
});
