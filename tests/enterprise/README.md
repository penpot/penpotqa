...
**Status: 69 implemented and passing live, 4 still `test.skip` stubs.**

| Qase IDs                                       | Qase suite                                                                                                                            | File                                                             |
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| 3235, 3236, 3336, 3324, 3413, 3437             | Cross-cutting: Enterprise Dashboard (direct) / Admin Console > Subscriptions & Billing / Admin Console (direct)                       | `billing-ui-flow/subscribe-and-trial-flow.spec.ts`               |
...
| 3342, 3343, 3344, 3345, 3346, 3348, 3349, 3626 | Admin Console > Sidebar Menu > Advanced Permissions > Move teams across organizations (Permission)                                    | `admin-console/advanced-permissions-move-teams.spec.ts`          |
| 3359, 3360, 3361, 3362, 3364                   | Admin Console > Sidebar Menu > Advanced Permissions > Send invitations (Permission)                                                   | `admin-console/advanced-permissions-send-invitations.spec.ts`    |
| 3569, 3571, 3573, 3574, 3576, 3577, 3579       | Admin Console > Sidebar Menu > Advanced Permissions > New team members (Permission)                                                   | `admin-console/advanced-permissions-new-team-members.spec.ts`    |
| 3120, 3123, 3127                               | Enterprise Dashboard > Organizations Dropdown > Organization Management > Leave Org                                                   | `dashboard-enterprise/organization-management-leave-org.spec.ts` |
...
| File                                                       | Covers                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `pages/dashboard/organization-page.ts`                     | Dashboard-side entry points: sidebar "+ Create org" button/promo widget, "Unlock Enterprise features" modal, org switcher dropdown, "Create organization" naming modal, and the manual activation-code flow (Qase 3437). Also `acceptOrgInviteFromInbox(email, orgName)` — see `helpers/accounts/` below.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `helpers/organizations/activate-enterprise-license.ts`     | `activateEnterpriseLicense(request)` — the RPC → licenses-manager `/api/activation-codes/create` → RPC path that grants Enterprise without Stripe. PRE/dev only (`LICENSES_MANAGER_URL`); see "How Enterprise entitlement actually works" above. `requestActivationCode(request)` exposes just the code-generation part (no redeem) for Qase 3437, which redeems it through the UI instead.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `helpers/accounts/`                                        | Every "get a logged-in session" primitive: `create-demo-user.ts` (`createDemoUser`, API-only), `create-activated-demo-user.ts` (`createActivatedDemoUser`, wraps it with `activateEnterpriseLicense` — shared by `enterpriseDemoAccountApiFixture` and `ownerAndInviteeActivatedTest`'s `ownerPage`), `login-as-demo-account.ts` (`loginAsDemoAccount`), `register-new-account.ts` (`registerNewAccount`, real Gmail-alias account), `create-invitee-session.ts` (`createOrgInviteeSession`/`createTeamInviteeSession`, a real second account in its own context — not a demo one, since a demo profile's inbox is unreadable and can't accept an org invite; both take an optional `ordinal` for a test creating more than one, and `createTeamInviteeSession` additionally requires an `InvitationRole`, folded into the generated name/email so distinct invitees stay distinguishable in logs/traces). Prefer `ownerAndInviteeTest`/`ownerAndInviteeActivatedTest` over calling either directly — it also handles cleanup. |
| `pages/admin-console/advanced-permissions-page.ts`         | The Advanced Permissions tab — 4 policy radio groups sharing one self-healing `selectPermission()`/`isPermissionSelected()` pair typed against `AdvancedPermissionValue`. New team members' "Organization members only" is the one exception: selecting it while the org has pending invitations to non-members raises an inline warning panel first (`clickOrganizationMembersOnlyExpectingWarning()`/`confirmApplyAndRevokeInvitations()`), rather than autosaving immediately like the other 3 groups.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `pages/dashboard/team-page.js` (Enterprise additions)      | Team Settings' "Team organization" section — add/remove/move a team between orgs, the org-picker's disabled/tooltip state and cancel-invitations warning, and the blocking "no organization qualifies" modal; plus the team invite dialog's permission-denied states (no-permission-to-invite message, the blocked-invitations replacement modal for a restricted "New team members" org).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `helpers/gmail.js` (Enterprise additions)                  | `getMessageSubject()`, plus `checkEnterpriseInviteText`/`checkEnterpriseInviteSubject` for the org-scoped invite template. `waitForMessageCount(page, email, count, timeoutSec?)` polls an inbox for at least `count` messages; `waitSecondMessage(page, email, timeoutSec?)` is a thin wrapper for the common `count = 2` case.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
